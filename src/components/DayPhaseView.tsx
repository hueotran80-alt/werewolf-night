...
interface Props {
  ...
  onSendChat: (text: string, channel: 'DAY_PUBLIC' | 'GHOST_PRIVATE') => void;
  onOpenMyCard: () => void;
}
...
export const DayPhaseView: React.FC<Props> = ({
  ...
  // Filter messages for daytime or ghost
  const relevantMessages = chatMessages.filter(
    (m) => m.channel === 'DAY_PUBLIC' || (m.channel === 'GHOST_PRIVATE' && !isAlive)
  );
    ...
    <div className="w-full flex-1 flex flex-col justify-between max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between p-4 rounded-3xl bg-amber-950/20 border border-amber-800/40 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          <div className="px-3.5 py-1.5 rounded-2xl bg-zinc-900 border border-zinc-800 font-mono text-xs font-bold text-amber-400">
            ⏳ {remainingSeconds}s
          </div>
          <button
            type="button"
      ...
      <div className="p-4 rounded-3xl bg-zinc-950/80 border border-zinc-800 shadow-xl space-y-2">
        ...
        {victims.length === 0 ? (
          <div className="p-3 rounded-2xl bg-emerald-950/30 border border-emerald-800/40 text-xs text-emerald-300 font-semibold flex items-center gap-2">
            ✨ Đêm qua là một đêm bình yên kỳ lạ! Không có nạn nhân nào thiệt mạng.
          </div>
        ) : (
        ...
        {silencedPlayers.length > 0 && (
          <div className="p-3 rounded-2xl bg-amber-950/30 border border-amber-800/50 text-xs text-amber-300 font-medium flex items-center gap-2">
            ...
            <span>
              🍃 <strong>Phong Ấn Câm Lặng:</strong> {silencedPlayers.map((p) => p.nickname).join(', ')} đã bị <strong>Liễu</strong> phong ấn đêm qua — không thể mở mic hay gửi tin nhắn chat trong ngày hôm nay!
            </span>
          </div>
        )}
      </div>

      {/* Town Hall Discussion Chat */}
      <div className="flex-1 flex flex-col bg-zinc-950/80 border border-zinc-800 rounded-3xl shadow-xl overflow-hidden min-h-[280px]">
        <div className="p-3 border-b border-zinc-800/80 bg-zinc-900/50 flex items-center justify-between px-4">
          <div className="flex items-center gap-2 text-xs font-bold text-zinc-300">
            ...
            <span>Kênh Hội đồng Làng {isAlive ? '(Công khai)' : '👻 (Cõi Âm)'}</span>
          </div>
          <span className="text-[10px] text-zinc-400">
            {room.players.filter((p) => p.isAlive).length} người sống
          </span>
        </div>
        ...
        {isSilenced ? (
          <div className="p-3.5 border-t border-zinc-800 bg-amber-950/20 flex items-center justify-center gap-2 text-xs text-amber-300 font-semibold">
            ...
            <span>🤐 Bạn đang bị Liễu phong ấn câm lặng! Không thể mở mic hoặc gửi chat trong ngày hôm nay.</span>
          </div>
        ) : (
